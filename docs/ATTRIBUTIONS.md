...
**[VibeMathed](https://vibemathed.com)** — The idea of giving the corpus a statistics page, and the choice to break it down by field and by what the model contributed, follow their example. The charts on [`/stats`](https://conjecture-hub-test.vercel.app/stats/) are our own: hand-rolled SVG in this repository’s own palette, over series computed from our YAML at build time and published at [`/index/series.json`](https://conjecture-hub-test.vercel.app/index/series.json). No figure, series or axis is taken from theirs, and we deliberately do not reproduce their significance-weighted and combined-years-open charts, because we hold neither number.
